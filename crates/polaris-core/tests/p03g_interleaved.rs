...
fn seed_mastery(
    ...
    engine
        .conn()
        .execute(
            "INSERT INTO mastery_states(concept_id, p_known, calib_gap, attempt_count, max_depth, updated_at)
             VALUES (?1, ?2, ?3, ?4, ?5, '2026-06-12T00:00:00Z')
             ON CONFLICT(concept_id) DO UPDATE SET
                p_known=excluded.p_known,
                calib_gap=excluded.calib_gap,
                attempt_count=excluded.attempt_count,
                max_depth=excluded.max_depth",
            (concept_id, p_known, calib_gap, attempt_count, max_depth),
        )
        .unwrap();
    engine
        .conn()
