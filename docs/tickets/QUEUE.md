...
状态：**P16J 已通过验收，待提交；P16I 已提交（`7cbbd0b`）；P16H 已提交（`81cd25f`）；P16G 已提交（`b558fae`）；P16G2 已提交（`2735169`）；P16G1 已提交（`825f8a7`）；P16D 已提交（`7a478cc`）；P16C 已提交（`7dc54f2`）；P16B 已提交（`60effc0`）；P16A 已提交（`fbcad7b`）；P15B 已提交（`a9fd13b`）**。任何时刻只允许 1 张票 In Progress。
...
- [x] **P16J 教学回合对象与上下文回取**（`TICKET_P16J_TEACHING_TURN_CONTEXT.md`）← 已通过验收，待提交；依赖 P01/P03H/P15B；为外部导师提供只读历史上下文并登记单次讲解事实，不放宽 strict-citation；服务环节：针对性补缺 → 验证真懂
