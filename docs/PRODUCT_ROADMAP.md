...
以下执行序是 2026-06-15 的产品路线图建议，P07A-P10A 已按 `docs/tickets/QUEUE.md` 转为正式票并完成。保留本节只作为依赖和设计理据记录，不再作为下一票认领入口。
...
1. 读 `AGENTS.md`、`docs/AI_RUNBOOK.md`、`SPEC.md` 与 `docs/tickets/QUEUE.md`。
2. 如果 QUEUE 有 In Progress 票，只续做那张票。
3. 如果 QUEUE 有未完成正式票，按 QUEUE 顺序认领；不要从本路线图的历史执行序自行认领。
