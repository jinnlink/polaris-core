...
3. 如果已有 In Progress 票，只续做那张票；如果没有，先看 QUEUE 是否有未完成正式票。若 QUEUE 无未完成正式票，不从旧路线图自行认领，向用户报告当前无可自动认领下一票并等待裁决。
...
读 QUEUE → 确认有未完成正式票 → 把票标 In Progress → 按票实现 → 跑验收（全绿）→ 在票尾写交付记录 → 等用户确认后 commit。若 QUEUE 无未完成正式票，先请用户裁决，不从历史路线图自行开工。
